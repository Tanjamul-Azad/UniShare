...
export default function NotificationsDropdown() {
  ...
  const { notifications, unreadNotificationsCount, markNotificationRead } =
    useSocket();
  ...
  const handleNotificationClick = (
    ...
    setIsOpen(false);
    navigate(linkUrl || getNotificationTarget(type));
  };
                ...
                <div className="divide-y divide-gray-50">
                  {notifications
                    ...
                    .map((notif) => (
                      ...
                      >
                        <div
                          className={`w-8 h-8 rounded-full flex items-center justify-center shrink-0 ${!notif.read ? "bg-white shadow-sm" : "bg-gray-100"}`}
                        >
                          {getIcon(notif.type)}
                        </div>
                        <div className="flex-1 min-w-0">
                          <p
                            className={`text-sm ${!notif.read ? "font-semibold text-gray-900" : "font-medium text-gray-800"}`}
                          >
                            {notif.title}
                          </p>
