      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-gray-500 mt-1">
            Swap UIU course packs, lab gear, and study tools with verified
            students.
          </p>
        </div>
        <div className="flex items-center gap-3">
          ...
          >
            ...
            Filters{" "}
            {activeFilterCount > 0 && (
              <span className="bg-indigo-100 text-indigo-700 text-xs rounded-full px-1.5 py-0.5 ml-1 flex items-center justify-center font-semibold">
                {activeFilterCount}
              </span>
            )}
          </button>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 font-body items-stretch">
        {loading ? (
          ...
          Array.from({ length: 8 }).map((_, i) => (
            <div
              key={i}
              className="flex flex-col h-full animate-pulse p-4 bg-white border border-gray-100 rounded-2xl shadow-sm"
            >
            ...
            >
              ...
              >
                  ...
                  <div className="absolute bottom-3 right-3 flex items-center gap-2 z-20">
                    <button
                      onClick={(e) => {
                        ...
                        e.stopPropagation();
                        item.sellerId &&
                          navigate(`/inbox?participant=${item.sellerId}`);
                      }}
                      className="p-2 bg-white/95 backdrop-blur-md rounded-full shadow-sm hover:scale-110 active:scale-95 transition-all text-gray-400 hover:text-indigo-600"
                    >
                      <MessageSquare className="w-4 h-4" />
                    </button>
                    <button
                      onClick={(e) => handleToggleFavorite(e, item.id)}
                      className="p-2 bg-white/95 backdrop-blur-md rounded-full shadow-sm hover:scale-110 active:scale-95 transition-all text-gray-400 hover:text-rose-500"
                    >
                      <Heart
                        className={cn(
                ...
                <div className="space-y-1 flex-1 flex flex-col">
                  <div className="flex items-start justify-between gap-2">
                    <h3 className="font-semibold text-gray-900 leading-tight line-clamp-2 group-hover:text-indigo-600 transition-colors">
                      {item.title}
                    </h3>
