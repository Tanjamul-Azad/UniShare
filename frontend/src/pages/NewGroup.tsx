        ...
        <form onSubmit={handleSubmit} className="space-y-8 relative z-10 font-['DM_Sans']">
          ...
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-2 gap-4">
              {[
                { id: 'share', label: 'Share (Co-Sub)', icon: Users, desc: 'Split the cost with others' },
          ...
          <div>
            ...
            <input type="text" required placeholder="e.g. Spotify Family, Netflix Premium, Adobe CC" className="w-full px-4 py-3 bg-gray-50 dark:bg-white/5 border border-gray-200 dark:border-white/10 rounded-xl focus:ring-2 focus:ring-indigo-500 outline-none text-gray-900 dark:text-white transition-all" />
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
            ...
            {listingType === 'share' && (
              <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }}>
                ...
                <input type="number" required min="2" max="10" placeholder="e.g. 4" className="w-full px-4 py-3 bg-gray-50 dark:bg-white/5 border border-gray-200 dark:border-white/10 rounded-xl focus:ring-2 focus:ring-indigo-500 outline-none text-gray-900 dark:text-white transition-all" />
              </motion.div>
            )}
            {listingType === 'sublet' && (
              <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }}>
                <label className="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2">Duration (Months)</label>
                <input type="number" required min="1" max="12" placeholder="e.g. 3" className="w-full px-4 py-3 bg-gray-50 dark:bg-white/5 border border-gray-200 dark:border-white/10 rounded-xl focus:ring-2 focus:ring-indigo-500 outline-none text-gray-900 dark:text-white transition-all" />
              </motion.div>
            )}
          </div>
          ...
          <button type="submit" className="w-full py-4 bg-gradient-to-r from-indigo-600 to-indigo-500 hover:from-indigo-700 hover:to-indigo-600 text-white rounded-xl font-semibold shadow-lg shadow-indigo-500/25 hover:shadow-indigo-500/40 transition-all duration-200 transform hover:-translate-y-0.5">
            {listingType === 'share' ? 'Create Co-Sub Group' : 'List Sublet'}
          </button>
        </form>
      </div>
    </motion.div>
