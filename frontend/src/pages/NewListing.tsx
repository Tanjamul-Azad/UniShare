...
import { createMarketplaceListing, getMarketplaceItemById, updateMarketplaceItem, type CreateMarketplaceListingInput, type MarketplaceItem } from '../lib/api';
...
export default function NewListing() {
  ...
  const {
    ...
    queryFn: () => (editId ? getMarketplaceItemById(editId) : Promise.resolve(undefined)),
    enabled: isEditing,
  ...
  const createListingMutation = useMutation({
    ...
    onError: (error: any) => {
      ...
      emitToast(error?.message || 'Unable to publish listing. Please try again.', 'error');
    },
  });

  const updateListingMutation = useMutation({
    mutationFn: ({ id, data }: { id: string; data: Partial<CreateMarketplaceListingInput> }) =>
      updateMarketplaceItem(id, data),
    onSuccess: async (item) => {
      await queryClient.invalidateQueries({ queryKey: ['marketplace-items'] });
      await queryClient.invalidateQueries({ queryKey: ['marketplace-item', item.id] });
      emitToast('Listing updated successfully.', 'success');
      navigate(`/marketplace/${item.id}`);
    },
    onError: (error: any) => {
      console.error("Update error:", error);
      emitToast(error?.message || 'Unable to update listing. Please try again.', 'error');
    },
  });
  ...
  if (isEditing && editItem && editItem.sellerId !== user?.id) {
    return (
      ...
      >
        <div className="py-10 text-center text-sm text-gray-500">Access denied.</div>
      </Modal>
    );
  }
  ...
  return (
    <>
      ...
      <Modal 
        ...
        description={isEditing ? "Update your listing details." : "Share, sell, or barter your stationary, books, and electronics."}
      >
          ...
          >
            <div className="space-y-2 text-center pointer-events-none">
              <Upload className={cn(
                ...
                isDragging ? "text-indigo-500" : "text-gray-400 group-hover:text-indigo-500"
              )} />
              <div className="flex text-sm text-gray-600 justify-center pointer-events-auto">
                <label htmlFor="file-upload" className="relative cursor-pointer rounded-md font-medium text-indigo-600 hover:text-indigo-500 focus-within:outline-none">
                  <span>{imageFileName ? 'Replace file' : 'Upload a file'}</span>
                ...
                <p className="pl-1">{imageFileName ? '' : 'or drag and drop'}</p>
              </div>
              <p className="text-xs text-gray-500 pointer-events-auto">
                {imageFileName ? (
                  <span className="font-semibold text-indigo-600 truncate max-w-xs inline-block align-bottom">{imageFileName}</span>
                ) : (
